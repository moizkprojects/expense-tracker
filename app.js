...
const state = {
  ...
  osmSuggestionTimer: null,
  osmSuggestionAbortController: null,
  isOsmSuggestionLoading: false
};
...
async function initializeApp() {
  ...
  try {
    ...
    hideLocationResults();
    updateEverything();
    setSaveStatus(`Rates ready (${state.locations.length} locations).`);
    elements.locationHelp.textContent = "Search a city, then run City Search to map it to a FY2026 rate.";
  } catch (error) {
    console.error(error);
    setSaveStatus("Rate file unavailable.");
    elements.locationHelp.textContent = "Could not load FY2026 workbook data on this browser.";
  }
}
...
function bindEvents() {
  ...
  elements.locationInput.addEventListener("input", () => {
    ...
    scheduleOsmSuggestions(elements.locationInput.value, true);
    switchLedgerIfNeeded();
    updateEverything();
  });
  ...
  elements.locationInput.addEventListener("keydown", (event) => {
    ...
    if (elements.locationResults.classList.contains("hidden")) {
      if (event.key === "Enter") {
        const exactTypedLocation = findExactTypedLocation();
        if (exactTypedLocation) {
          event.preventDefault();
          pickLocation(exactTypedLocation.label);
          return;
        }
    ...
    if (event.key === "Enter") {
      ...
      if (state.hasManualLocationNavigation && state.visibleLocationResults.length) {
        ...
        if (navigated) {
          pickOsmSuggestionByIndex(resolvedIndex);
        }
        return;
      }
...
function findExactTypedLocation() {
...
async function attemptOsmCountyLookup(preferredOsmResult = null, queryOverride = "") {
  if (!state.workbookReady || state.isOsmLookupRunning) {
    return;
  }

  const query = String(queryOverride || elements.locationInput.value || "").trim();
  if (!query) {
    elements.locationHelp.textContent = "Enter a city or location first.";
    return;
  }
  ...
  setSaveStatus("Searching city...");
  elements.locationHelp.textContent = "Looking up city with OpenStreetMap and matching FY2026 rates.";

  try {
    const resolution = await resolveWorkbookLocationFromOsm(query, preferredOsmResult);

    if (resolution.location) {
      pickLocation(resolution.location.label);

      if (resolution.matchType === "county") {
        elements.locationHelp.textContent = `Matched by county: ${resolution.countyDisplay || "Unknown"} (${resolution.stateCode}).`;
      } else {
        elements.locationHelp.textContent = `Matched by city: ${resolution.cityDisplay || query} (${resolution.stateCode}).`;
      }
    ...
    if (resolution.standardConus) {
      ...
      elements.locationHelp.textContent = "No city/county workbook match. Standard CONUS applied.";
      setSaveStatus(`No exact match for "${query}". Standard CONUS applied.`);
      return;
    }

    elements.locationHelp.textContent = "No workbook match found. Try another nearby city.";
    setSaveStatus("No workbook match found.");
  } catch (error) {
    console.error(error);
    const message = error && error.name === "AbortError"
      ? "City search timed out. Try again."
      : "City search failed. Check connection.";
...
async function resolveWorkbookLocationFromOsm(query, preferredOsmResult = null) {
  const osmResults = preferredOsmResult
    ? [preferredOsmResult]
    : await searchOpenStreetMap(query, 8);
...
async function searchOpenStreetMap(query, limit = 8) {
  ...
  try {
    const url = buildOsmSearchUrl(query, limit);
...
async function searchOpenStreetMapSuggestions(query) {
  if (state.osmSuggestionAbortController) {
    state.osmSuggestionAbortController.abort();
  }

  const controller = new AbortController();
  state.osmSuggestionAbortController = controller;
  const timeoutId = window.setTimeout(() => {
    controller.abort();
  }, 6500);

  try {
    const url = buildOsmSearchUrl(query, 6);
    const response = await fetch(url, {
      method: "GET",
      headers: {
        Accept: "application/json",
    ...
    if (!response.ok) {
      throw new Error(`OpenStreetMap suggestion search failed with status ${response.status}.`);
    }
    ...
    return payload
      .filter((item) => item && item.address && typeof item.address === "object")
      .filter((item) => isCityLikeOsmResult(item));
  } finally {
    window.clearTimeout(timeoutId);
    state.osmSuggestionAbortController = null;
  }
}

function buildOsmSearchUrl(query, limit = 8) {
...
function isCityLikeOsmResult(result) {
  ...
  return type === "city" || type === "town" || type === "village" || type === "administrative";
}

function buildOsmSuggestionLabel(result) {
  const address = result && result.address ? result.address : {};
  ...
  const county = firstNonEmpty([address.county, address.state_district]);
  const stateCode = extractStateCode(address);
  const stateText = firstNonEmpty([stateCode, address.state]);

  if (city && stateText && county) {
    return `${city}, ${stateText} (${county})`;
  }
  if (city && stateText) {
  ...
  return String(result.display_name || "OpenStreetMap result");
}
...
function findBestCityLocation(stateLocations, cityNormalized, typedNormalized) {
  ...
  stateLocations.forEach((location) => {
    const destinationNormalized = normalizeGeoName(location.destination);
    const labelNormalized = normalizeGeoName(location.label);
    const destinationTokens = getDestinationTokensForLocation(location);
    let score = 0;

    if (destinationTokens.includes(target)) {
      score += 180;
    } else if (target && destinationNormalized.includes(target)) {
      score += 120;
    } else if (target && target.length >= 4 && target.includes(destinationNormalized)) {
      score += 70;
    }

    if (target && labelNormalized.includes(target)) {
      score += 40;
    }
...
function getDestinationTokensForLocation(location) {
  const source = String(location.destination || "").trim();
  if (!source) {
    return [];
  }
...
function renderRateCard(rateRecord, location) {
  if (!location || !rateRecord) {
    elements.rateTitle.textContent = "No Matched Rate";
    elements.rateSeason.textContent = "Search a city and run City Search, or pick a workbook location.";
    elements.baseRate.textContent = "$0.00";
    elements.effectiveRate.textContent = "$0.00";
    elements.lodgingRate.textContent = "$0.00";

    if (elements.locationInput.value.trim()) {
      elements.locationHelp.textContent = "Use City Search to map this city to workbook city/county.";
    } else {
      elements.locationHelp.textContent = "Search any US city, then map it with City Search.";
    }
...
function renderSummary(location, rateRecord, effectiveRate, totalSpent, difference) {
  ...
  if (!location || !rateRecord) {
    ...
    elements.summaryTitle.textContent = "Set Date + Location";
    elements.summaryCaption.textContent = "Daily budget appears after a city/workbook match.";
    return;
  }
...
function renderHistory() {
  if (!state.ledgers.length) {
    elements.history.innerHTML = '<div class="empty-state">Saved days will appear here.</div>';
    return;
  }
  ...
  elements.history.innerHTML = state.ledgers.map((ledger) => {
    ...
    return `
      <div class="history-item ${balanceClass}${activeClass}">
        ...
        <button class="ghost-button" type="button" data-open-ledger="${escapeHtml(ledger.id)}">Open</button>
...
function scheduleOsmSuggestions(rawQuery, forceShow = false) {
  const query = String(rawQuery || "").trim();

  if (state.osmSuggestionTimer) {
    window.clearTimeout(state.osmSuggestionTimer);
    state.osmSuggestionTimer = null;
  }

  if (!query || query.length < 2) {
    state.visibleLocationResults = [];
    hideLocationResults();
    return;
  }

  state.osmSuggestionTimer = window.setTimeout(() => {
    state.osmSuggestionTimer = null;
    void loadOsmSuggestions(query, forceShow);
  }, 260);
}

async function loadOsmSuggestions(query, forceShow = false) {
  state.isOsmSuggestionLoading = true;
  renderLocationResults(forceShow);

  try {
    const results = await searchOpenStreetMapSuggestions(query);
    const latestQuery = String(elements.locationInput.value || "").trim();
    if (latestQuery !== query) {
      return;
    }

    state.visibleLocationResults = results.map((result) => {
      return {
        label: buildOsmSuggestionLabel(result),
        osmResult: result
      };
    });
    ...
    state.isOsmSuggestionLoading = false;
    renderLocationResults(forceShow);
  }
}

function renderLocationResults(forceShow = false) {
  if (!state.workbookReady) {
    hideLocationResults();
    return;
  }

  const normalizedQuery = String(elements.locationInput.value || "").trim();
  ...
  if (state.isOsmSuggestionLoading) {
    elements.locationResults.innerHTML = '<div class="location-empty">Searching OpenStreetMap...</div>';
    elements.locationResults.classList.remove("hidden");
    return;
  }

  if (!state.visibleLocationResults.length) {
    elements.locationResults.innerHTML = '<div class="location-empty">No location found. Press Enter to map directly.</div>';
    elements.locationResults.classList.remove("hidden");
    return;
  }

  elements.locationResults.innerHTML = state.visibleLocationResults.map((item, index) => {
    const activeClass = index === state.activeLocationResultIndex ? " active" : "";
    return `
      <button class="location-result${activeClass}" type="button" data-pick-location-index="${index}">
        ${escapeHtml(item.label)}
      </button>
    `;
  }).join("");

  Array.from(elements.locationResults.querySelectorAll("[data-pick-location-index]")).forEach((button) => {
...
function bindLocationPickEvents(button) {
  ...
  const handlePick = (event) => {
    ...
    isHandled = true;
    const index = Number.parseInt(button.dataset.pickLocationIndex || "", 10);
    if (Number.isFinite(index)) {
      pickOsmSuggestionByIndex(index);
    }
  };
...
function pickOsmSuggestionByIndex(index) {
  const picked = state.visibleLocationResults[index];
  if (!picked || !picked.osmResult) {
    return;
  }

  elements.locationInput.value = picked.label;
  hideLocationResults();
  void attemptOsmCountyLookup(picked.osmResult, picked.label);
}
